...
class PvrArtwork(object):
    ...
    def get_pvr_artwork(self, title, channel, genre="", manual_select=False, ignore_cache=False):
        ...
        if not proceed_lookup and manual_select:
            ...
            proceed_lookup = xbmcgui.Dialog().yesno(line1=self.artutils.addon.getLocalizedString(32027), heading=xbmc.getLocalizedString(750))
    ...
    def pvr_proceed_lookup(self, title, channel, genre):
        ...
        for item in self.artutils.addon.getSetting("pvr_art_ignore_titles").split("|"):
            if item.lower() == title.lower():
                log_msg("PVR artwork - filter active for title: %s channel: %s genre: %s --> Title is in list of titles to ignore"%(title, channel, genre))
                return False
        for item in self.artutils.addon.getSetting("pvr_art_ignore_channels").split("|"):
            if item.lower() == channel.lower():
                log_msg("PVR artwork - filter active for title: %s channel: %s genre: %s --> Channel is in list of channels to ignore"%(title, channel, genre))
                return False
        for item in self.artutils.addon.getSetting("pvr_art_ignore_genres").split("|"):
            if genre and item.lower() in genre.lower():
                log_msg("PVR artwork - filter active for title: %s channel: %s genre: %s --> Genre is in list of Genres to ignore"%(title, channel, genre))
                return False
        if self.artutils.addon.getSetting("pvr_art_ignore_commongenre") == "true":
            ...
            for kodi_string in kodi_strings:
                ...
                if (genre and (genre in kodi_string or kodi_string in genre)) or kodi_string in title:
                    log_msg("PVR artwork - filter active for title: %s channel: %s genre: %s --> Common genres like weather/sports are set to be ignored"%(title, channel, genre))
                    return False
        if self.artutils.addon.getSetting("pvr_art_recordings_only") == "true":
            recordings = self.lookup_local_recordings(title)
            if not recordings:
                log_msg("PVR artwork - filter active for title: %s channel: %s genre: %s --> PVR artwork is only enabled for recordings"%(title, channel, genre))
                return False
        return True
    ...
    def lookup_tvdb(self, searchtitle, channel, manual_select=False):
        ...
        if match_results and manual_select:
            ...
            for item in match_results:
                ...
                listitems.append(listitem)
            w = DialogSelect( "DialogSelect.xml", "", listing=listitems, window_title="%s - TVDB"%xbmc.getLocalizedString(283) )
            w.doModal()
            selected_item = w.result
            del w
            if selected_item != -1:
