...
class PvrArtwork(object):
    ...
    def manual_set_pvr_artwork(self, title, channel, genre):
        ...
        # import replaced with code copied from utils.py - following 2 lines commented out
        # from utils import manual_set_artwork
        # changemade, artwork = manual_set_artwork(details["art"], "pvr")
        
        # set up variables required by code copied from utils.py
        mediatype = "pvr"
        header = None
        artwork = details["art"]
            ...
            if selected_item == -1:
                ...
                if image and selected_item == 1:
                    ...
                    if image:
                        artwork[label] = image
                        changemade = True
        # ========= end of code copied from utils.py ========
        if changemade:
            details["art"] = artwork
